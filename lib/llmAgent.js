...
import { notifyOwnerAboutBooking } from "./telegram.js";
...
function getTodayInfo(timezone) {
  const now = new Date();
  // en-CA форматирует дату как YYYY-MM-DD "из коробки" — то, что нужно для get_available_slots
...
function buildSystemPrompt(config) {
  const { isoDate, weekday } = getTodayInfo(config.timezone);
...
Сьогоднішня дата: ${isoDate} (${weekday}). Використовуй її, щоб порахувати "завтра", "післязавтра", "у п'ятницю" тощо — у get_available_slots і book_appointment завжди передавай дату у форматі YYYY-MM-DD.
